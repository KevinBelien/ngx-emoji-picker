...
describe('TranslatePipe', () => {
    ...
    it('should return the translation for a given key', () => {
        ...
        jest.spyOn(translationService, 'getTranslationByKey').mockReturnValue(translatedValue);

        const result = pipe.transform(key);

        expect(result).toBe(translatedValue);
        expect(translationService.getTranslationByKey).toHaveBeenCalledWith(key);
    });

    it('should return the key itself if translation is not found', () => {
        const key = 'nonexistent_key';
        jest.spyOn(translationService, 'getTranslationByKey').mockReturnValue(undefined);
