...
export class SkintoneSwatchPickerComponent {
    ...
    selectedColor = computed(() => this.getColorBySkintone(this.selectedSkintone()));

    private getColorBySkintone = (skintone: Skintone): SkintoneColor => this.skintoneColors.find((color) => color.skintone === skintone) ?? this.skintoneColors[0];
    ...
    protected handleClick = (skintoneColor: SkintoneColor, event: Event) => {
    ...
    protected getPosition = (index: number) => {
        ...
        return `translateX(${position}px) ${this.selectedColor() === this.skintoneColors[index] ? 'scale(1.2)' : ''}`;
    };
}
