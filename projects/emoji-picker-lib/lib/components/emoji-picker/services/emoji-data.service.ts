...
export class EmojiDataService {
    ...
    constructor() {
        this.globalSkintoneSetting.update(() => this.fetchGlobalSkintone());

        this.individualSkintones.update(() => this.emojiStorageService.fetchIndividualEmojisSkintones());

        this.recentEmojis.update(() => this.fetchRecentEmojisFromStorage());

        this.frequentEmojis.update(() => this.fetchFrequentEmojisFromStorage());
    }
     ...
     */
    fetchGlobalSkintone = (): Skintone => this.emojiStorageService.fetchGlobalSkintone();
     ...
     */
    fetchSkintoneFromEmoji = (emoji: Emoji, skintone: Skintone): string => {
        if (!emoji.skintones) {
            return emoji.value;
        }
        const skintoneObj = emoji.skintones.find((s) => s.skintone === skintone);
     ...
     */
    fetchEmojisByIds = (emojiIds: string[]): Emoji[] => emojiIds.map((id) => this.fetchEmojiById(id)).filter((emoji): emoji is Emoji => !!emoji);
     ...
     */
    generateEmojiMap = (emojis: Emoji[], skintoneSetting: SkintoneSetting, globalSkintoneSetting: Skintone, individualEmojiSkintones: IndividualEmojiSkintone[]): Map<string, Emoji> =>
        new Map(emojis.map((emoji) => [emoji.id, this.getEmojiBySkintoneSettings(emoji, skintoneSetting, globalSkintoneSetting, individualEmojiSkintones)]));
     ...
     */
    getEmojiBySkintoneSettings = (emoji: Emoji, skintoneSetting: SkintoneSetting, globalSkintoneSetting: Skintone, individualEmojisSkintones: IndividualEmojiSkintone[]): Emoji => {
        if (!emoji.skintones || emoji.skintones.length === 0 || skintoneSetting === 'none') {
            return emoji;
        }

        if (skintoneSetting === 'individual') {
            const individualEmoji = individualEmojisSkintones.find((e) => e.emojiId === emoji.id);

            return individualEmoji ? Object.assign({ ...emoji }, { value: individualEmoji.emojiValue }) : emoji;
        }

        const alternativeSkintone = emoji.skintones.find((skintone) => skintone.skintone === globalSkintoneSetting);

        return Object.assign(
            { ...emoji },
            {
                value: alternativeSkintone ? alternativeSkintone.value : emoji.value
            }
        );
    };
     ...
     */
    hasEmojiSkintones = (emoji: Emoji): boolean => !!emoji.skintones && emoji.skintones.length > 0;
    ...
    fetchRecentEmojisFromStorage = (): Emoji[] => {
        const emojiIds = this.emojiStorageService.retrieveFromStorage<string>('recent');

        return this.fetchEmojisByIds(emojiIds).map((emoji) => Object.assign({ ...emoji }, { category: 'suggestions' }));
    };
    ...
    fetchFrequentEmojisFromStorage = (): Emoji[] => {
        const frequentEmojis = this.emojiStorageService.fetchFrequentEmojis();

        return this.fetchEmojisByIds(frequentEmojis.map((frequentEmoji) => frequentEmoji.id)).map((emoji) => Object.assign({ ...emoji }, { category: 'suggestions' }));
    };
    ...
    addEmojiToRecents = (id: string): void => {
        const emojiIds = this.emojiStorageService.prependIdToStorage('recent', id);
        const emojis = this.fetchEmojisByIds(emojiIds).map((emoji) => Object.assign({ ...emoji }, { category: 'suggestions' }));
    ...
    increaseEmojiFrequency = (id: string): void => {
        const frequentEmojis = this.emojiStorageService.increaseEmojiFrequency(id);

        const emojis = this.fetchEmojisByIds(frequentEmojis.map((frequentEmoji) => frequentEmoji.id)).map((emoji) => Object.assign({ ...emoji }, { category: 'suggestions' }));
