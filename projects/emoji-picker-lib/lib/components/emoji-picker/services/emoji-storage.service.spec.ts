...
import { FrequentEmoji, IndividualEmojiSkintone, Skintone } from '../models';
...
describe('EmojiStorageService', () => {
    ...
    describe('retrieveFromStorage', () => {
        it('should retrieve data from localStorage', () => {
            const mockData: FrequentEmoji[] = [{ id: '1', count: 10, dateInMs: 12345 }];
            localStorage.setItem(service.STORAGE_CONFIG.frequent.key, JSON.stringify(mockData));

            const result = service.retrieveFromStorage<FrequentEmoji>('frequent');
    ...
    describe('storeInStorage', () => {
        it('should store data in localStorage', () => {
            ...
            const storedData = localStorage.getItem(service.STORAGE_CONFIG.frequent.key);
    ...
    describe('prependToStorage', () => {
        it('should prepend data to the existing array in localStorage', () => {
            ...
            localStorage.setItem(service.STORAGE_CONFIG.recent.key, JSON.stringify(initialData));

            const newData: string = '2';
            const result = service.prependIdToStorage('recent', newData);

            expect(result).toEqual([newData, ...initialData]);
            expect(localStorage.getItem(service.STORAGE_CONFIG.recent.key)).toEqual(JSON.stringify([newData, ...initialData]));
        });
        ...
        it('should not create duplicates when prepending existing id', () => {
            ...
            const result = service.prependIdToStorage('recent', initialData[0]);
        ...
        it('should not exceed limit when prepending data', () => {
            const initialData = Array.from({ length: 100 }, (_, index) => index.toString());
            ...
            expect(localStorage.getItem(service.STORAGE_CONFIG.recent.key)).toEqual(JSON.stringify(result));
        });
    });
    ...
    describe('increaseEmojiFrequency', () => {
        it('should increase frequency of an existing emoji', () => {
            const mockData: FrequentEmoji[] = [{ id: '1', count: 5, dateInMs: 12345 }];
            localStorage.setItem(service.STORAGE_CONFIG.frequent.key, JSON.stringify(mockData));
        ...
        it('should add a new emoji if it does not exist', () => {
            ...
            expect(result).toEqual(expect.arrayContaining([{ id: '1', count: 5, dateInMs: 12345 }, expect.objectContaining({ id: '2', count: 1 })]));
        });
    });
    ...
    describe('fetchIndividualEmojisSkintones', () => {
        it('should return individual skintone settings from localStorage', () => {
            const mockData: IndividualEmojiSkintone[] = [{ emojiId: '1', emojiValue: 'skin-tone-2' }];

            localStorage.setItem(service.STORAGE_CONFIG.emojisSkintone.key, JSON.stringify(mockData));
    ...
    describe('updateEmojiSkintone', () => {
        it('should update the skintone of an existing emoji', () => {
            const initialData: IndividualEmojiSkintone[] = [{ emojiId: '1', emojiValue: 'skin-tone-2' }];
            localStorage.setItem(service.STORAGE_CONFIG.emojisSkintone.key, JSON.stringify(initialData));
    ...
    describe('fetchGlobalSkintone', () => {
        it('should return the global skintone from localStorage', () => {
            localStorage.setItem(service.STORAGE_CONFIG.globalSkintone.key, 'medium-light');
        ...
        it('should return default if the skintone in localStorage is invalid', () => {
            localStorage.setItem(service.STORAGE_CONFIG.globalSkintone.key, 'invalid-skintone');
            const result = service.fetchGlobalSkintone();
            expect(result).toBe('default');
        });
    });
    ...
    describe('updateGlobalSkintone', () => {
        it('should update the global skintone in localStorage', () => {
            ...
            expect(localStorage.getItem(service.STORAGE_CONFIG.globalSkintone.key)).toBe('dark');
        });

        it('should not update global skintone if the skintone is invalid', () => {
            service.updateGlobalSkintone('invalid-skintone' as Skintone);
            expect(localStorage.getItem(service.STORAGE_CONFIG.globalSkintone.key)).toBeNull();
        });
    });
});
