...
export class RippleDirective implements AfterViewInit, OnDestroy {
    ...
    ngAfterViewInit() {
        this.pointerDownListener = this.renderer.listen(this.el.nativeElement, 'pointerdown', this.onPointerDown.bind(this));
    }
    ...
    private onPointerDown(evt: MouseEvent) {
        ...
        const diameter = Math.max(this.hostEl.offsetWidth, this.hostEl.offsetHeight);
