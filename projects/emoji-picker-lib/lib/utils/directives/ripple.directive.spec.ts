...
describe('RippleDirective', () => {
    ...
    beforeEach(() => {
        TestBed.configureTestingModule({
            ...
            providers: [Renderer2, { provide: DOCUMENT, useValue: document }]
        }).compileComponents();
    ...
    it('should remove the previous ink element before creating a new one', () => {
        ...
        let inkElement = directiveElement.querySelector('.ch-ink') as HTMLElement;
        expect(inkElement).not.toBeNull();

        event = new MouseEvent('pointerdown');
        directiveElement.dispatchEvent(event);

        const newInkElement = directiveElement.querySelector('.ch-ink') as HTMLElement;
