import { DestroyRef, Directive, ElementRef, inject, input, OnDestroy, OnInit, output, Renderer2, signal } from '@angular/core';
import { takeUntilDestroyed } from '@angular/core/rxjs-interop';
import { Subject, timer } from 'rxjs';
import { switchMap, takeUntil, tap } from 'rxjs/operators';
import { ClickActionType, ClickEvent, PointerDeviceType, TouchHoldEvent } from '../models';
...
export class ClickTouchHoldDirective implements OnInit, OnDestroy {
    ...
    private addEventListeners = (): void => {
        ...
        this.eventListeners = [
            this.renderer.listen(nativeElement, 'pointerdown', (event: PointerEvent) => {
                this.onPointerDown(event);
            }),
            this.renderer.listen(nativeElement, 'pointerup', (event: PointerEvent) => {
                this.onPointerUp(event);
            }),

            this.renderer.listen(nativeElement, 'scroll', () => this.scroll$.next()),

            this.renderer.listen(nativeElement, 'keyup', (event: KeyboardEvent) => this.onKeyUp(event))
        ];
    };
    ...
    private onPointerDown = (event: PointerEvent): void => {
        ...
        const targetElement = this.findElementByAttribute(event.target as HTMLElement);
        if (targetElement) {
            this.pointerDownTarget.set(targetElement);
            this.pointerDownDataAttribute.set(this.getAttributeValue(targetElement));
    ...
    private onKeyUp = (event: KeyboardEvent): void => {
        ...
        if (this.isClickTriggerKey(event.key) && targetElement) {
            ...
            this.onClick.emit({
                ...
                targetElement,
                data,
                action: event.key === 'Enter' ? ClickActionType.ENTER : ClickActionType.SPACE
            });
        }
    };
    ...
    private handlePointerUp = (event: PointerEvent): void => {
        ...
        this.onClick.emit({
            ...
            action: event.pointerType === PointerDeviceType.MOUSE ? (event.button === 0 ? ClickActionType.LEFTCLICK : ClickActionType.RIGHTCLICK) : undefined
        });
    };

    private findElementByAttribute = (targetElement: HTMLElement): HTMLElement | null => {
        const dataAttribute = this.dataAttribute();
        while (targetElement && dataAttribute && !targetElement.hasAttribute(dataAttribute) && targetElement.parentElement) {
            targetElement = targetElement.parentElement;
        }
        return targetElement;
    };

    private getAttributeValue = (targetElement: HTMLElement): string | null => {
        const dataAttribute = this.dataAttribute();
        if (!targetElement || !dataAttribute) return null;
        const elementWithAttribute = this.findElementByAttribute(targetElement);
        return elementWithAttribute ? elementWithAttribute.getAttribute(dataAttribute) : null;
    };

    private isTargetDataAttributeMatch = (targetElement: HTMLElement, data?: string | null): boolean => {
        const dataAttribute = this.dataAttribute();

        return !dataAttribute || (targetElement.hasAttribute(dataAttribute) && this.pointerDownDataAttribute() === data);
    };

    private isPointerDownValid = (event: PointerEvent): boolean => (event.pointerType === PointerDeviceType.MOUSE && [0, 2].includes(event.button)) || (event.pointerType !== PointerDeviceType.MOUSE && event.button === 0);

    private isPointerUpValid = (event: PointerEvent): boolean => {
        const targetElement = this.findElementByAttribute(event.target as HTMLElement);
        if (!targetElement) return false;

        const pointerUpDataAttribute = this.getAttributeValue(targetElement);
        return this.pointerDownDataAttribute() === pointerUpDataAttribute;
    };

    private isClickTriggerKey = (key: string): boolean => ['ENTER', ' '].includes(key.toUpperCase());
}
