...
import { AfterContentChecked, AfterViewChecked, booleanAttribute, Directive, ElementRef, inject, input, PLATFORM_ID, signal } from '@angular/core';
...
export class AutofocusDirective implements AfterContentChecked, AfterViewChecked {
    ...
    startAutoFocus() {
        if (isPlatformBrowser(this.platformId) && this.autofocus()) {
            setTimeout(() => {
                const focusableElements = this.getFocusableElements(this.host?.nativeElement);
    ...
    getFocusableElements(element: HTMLElement): HTMLElement[] {
        ...
        const focusableElements = Array.from(element.querySelectorAll<HTMLElement>(selectors.join(',')));

        // Filter out elements that are not actually visible or cannot be focused
        const visibleFocusableElements = focusableElements.filter((el) => this.isVisible(el));
