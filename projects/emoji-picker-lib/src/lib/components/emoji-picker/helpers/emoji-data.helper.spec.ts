...
describe('EmojiDataHelper', () => {
    ...
    describe('groupEmojisByCategory', () => {
        it('should group emojis by their categories', () => {
            const groupedEmojis = EmojiDataHelper.groupEmojisByCategory(sampleEmojis);
    ...
    describe('filterEmojisByCategories', () => {
        it('should filter emojis by the included categories', () => {
            const filteredEmojis = EmojiDataHelper.filterEmojisByCategories(sampleEmojis, ['animals-nature', 'food-drink']);

            expect(filteredEmojis).toHaveLength(3);
            expect(filteredEmojis.some((emoji) => emoji.category === 'animals-nature')).toBe(true);
            expect(filteredEmojis.some((emoji) => emoji.category === 'food-drink')).toBe(true);
        });

        it('should return an empty array when no categories match', () => {
            const filteredEmojis = EmojiDataHelper.filterEmojisByCategories(sampleEmojis, ['flags']);
    ...
    describe('filterAndSortEmojis', () => {
        it('should filter and then sort emojis by their categories and order', () => {
            const sortedEmojis = EmojiDataHelper.filterAndSortEmojis(sampleEmojis, ['animals-nature', 'food-drink', 'smileys-people']);
    ...
    describe('sortEmojis', () => {
        it('should sort emojis based on the provided category order and emoji order', () => {
            const sortedEmojis = EmojiDataHelper.sortEmojis(sampleEmojis, ['food-drink', 'smileys-people', 'animals-nature', 'travel-places']);
