...
describe('EmojiDataService', () => {
    ...
    beforeEach(() => {
        ...
        emojiStorageService = TestBed.inject(EmojiStorageService) as jest.Mocked<EmojiStorageService>;
    });
    ...
    it('should add an emoji to recents and update storage', () => {
        ...
        emojiStorageService.prependIdToStorage.mockReturnValue(recentEmojiIds);

        // Call the method to add the emoji to recents
        service.addEmojiToRecents(id);

        // Verify that fetchEmojisByIds was called correctly
        const fetchedEmojis = service.fetchEmojisByIds(recentEmojiIds).map((emoji) => ({
            ...emoji,
        ...
        expect(emojiStorageService.prependIdToStorage).toHaveBeenCalledWith('recent', id);
    });

    it('should increase emoji frequency and update frequent emojis in storage', () => {
        const id = mockEmojis[0].id;
        const frequentEmojis: FrequentEmoji[] = [{ id, count: 1, dateInMs: Date.now() }];

        // Mock the return value of increaseEmojiFrequency to return the frequentEmojis array
        emojiStorageService.increaseEmojiFrequency.mockReturnValue(frequentEmojis);

        // Call the method to increase the emoji frequency
        service.increaseEmojiFrequency(id);

        // Verify that fetchEmojisByIds was called correctly
        const fetchedEmojis = service.fetchEmojisByIds([id]).map((emoji) => ({
            ...emoji,
            category: 'suggestions'
        }));
        ...
        // Ensure the storage service method was called with the correct ID
        expect(emojiStorageService.increaseEmojiFrequency).toHaveBeenCalledWith(id);
    });
    ...
    it('should correctly set the global emoji skintone', () => {
        ...
        expect(emojiStorageService.updateGlobalSkintone).toHaveBeenCalledWith(skintone);
        expect(service.globalSkintoneSetting()).toBe(skintone);
    });

    it('should generate an emoji map with skintone settings', () => {
        service.emojis.set(mockEmojis);
        const map = service.generateEmojiMap(mockEmojis, 'global', 'medium', []);
    ...
    it('should apply skintone settings correctly', () => {
        ...
        const result = service.getEmojiBySkintoneSettings(emoji, 'global', skintoneSetting, []);
