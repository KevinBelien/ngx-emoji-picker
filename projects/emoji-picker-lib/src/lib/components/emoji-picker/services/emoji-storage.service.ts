...
import { FrequentEmoji, IndividualEmojiSkintone, isValidSkintone, Skintone } from '../models';
...
export class EmojiStorageService {
     ...
     */
    retrieveFromStorage = <T>(storageKey: keyof typeof this.STORAGE_CONFIG): T[] => {
        const response = localStorage.getItem(this.STORAGE_CONFIG[storageKey].key);
     ...
     * @returns {void}
     */
    storeInStorage = <T>(storageKey: keyof typeof this.STORAGE_CONFIG, data: T): void => {
        localStorage.setItem(this.STORAGE_CONFIG[storageKey].key, JSON.stringify(data));
    };
     ...
     */
    prependIdToStorage = (storageKey: keyof typeof this.STORAGE_CONFIG, id: string): string[] => {
    ...
    fetchFrequentEmojis = (): FrequentEmoji[] => {
        const frequentEmojis = this.retrieveFromStorage<FrequentEmoji>('frequent');
    ...
    increaseEmojiFrequency = (id: string): FrequentEmoji[] => {
        ...
        let frequentEmojis = this.retrieveFromStorage<FrequentEmoji>('frequent');

        const emojiIndex = frequentEmojis.findIndex((emoji) => emoji.id === id);
     ...
     */
    sortFrequentEmojis = (emojis: FrequentEmoji[], sortDateDescending: boolean = false): FrequentEmoji[] =>
        emojis.sort((a, b) => {
            if (b.count !== a.count) {
                return b.count - a.count;
            }
            return sortDateDescending ? b.dateInMs - a.dateInMs : a.dateInMs - b.dateInMs;
        });
     ...
     */
    fetchIndividualEmojisSkintones = (): IndividualEmojiSkintone[] => this.retrieveFromStorage<IndividualEmojiSkintone>('emojisSkintone');
     ...
     */
    updateEmojiSkintone = (emojiId: string, emojiValue: string): IndividualEmojiSkintone[] => {
        const storageEmojiSkintones = this.fetchIndividualEmojisSkintones();

        const dto: IndividualEmojiSkintone = { emojiId, emojiValue };

        const index = storageEmojiSkintones.findIndex((record) => record.emojiId === emojiId);
        ...
        this.storeInStorage<IndividualEmojiSkintone[]>('emojisSkintone', storageEmojiSkintones);
    ...
    fetchGlobalSkintone = (): Skintone => {
        const response = localStorage.getItem(this.STORAGE_CONFIG.globalSkintone.key);
        return !!response && isValidSkintone(response) ? (response as Skintone) : 'default';
    };
    ...
    updateGlobalSkintone = (skintone: Skintone): void => {
        ...
        localStorage.setItem(this.STORAGE_CONFIG.globalSkintone.key, skintone);
    };
}
