...
describe('SkintoneSwatchPickerComponent', () => {
    ...
    it('should render the correct number of skintone swatches', () => {
        const swatches = fixture.debugElement.queryAll(By.css('.ch-color-swatch'));
    ...
    it('should highlight the selected skintone', () => {
        ...
        const selectedSwatch = fixture.debugElement.query(By.css('.ch-color-swatch.selected'));
        expect(selectedSwatch).not.toBeNull();

        const expectedColor = mockSkintoneColors.find((color) => color.skintone === 'medium-dark')?.color;
        const rgbColor = window.getComputedStyle(selectedSwatch.nativeElement).backgroundColor;
    ...
    it('should emit event when a skintone swatch is clicked', () => {
        ...
        const selectionChangedSpy = jest.spyOn(component.onSelectionChanged, 'emit');

        const swatch = fixture.debugElement.queryAll(By.css('.ch-color-swatch'))[2];

        swatch.triggerEventHandler('click', new Event('click'));
        fixture.detectChanges();

        expect(selectionChangedSpy).toHaveBeenCalledWith(selectedSkintone);
    ...
    it('should correctly position swatches when open', () => {
        ...
        swatches.forEach((swatch, index) => {
            ...
            const expectedTransform = `translateX(-${index * (component.size + component.itemPadding)}px)`;

            if (component.selectedColor().skintone === mockSkintoneColors[index].skintone) {
    ...
    it('should scale up the selected skintone swatch', () => {
        ...
        const transformStyle = selectedSwatch.nativeElement.style.transform;
