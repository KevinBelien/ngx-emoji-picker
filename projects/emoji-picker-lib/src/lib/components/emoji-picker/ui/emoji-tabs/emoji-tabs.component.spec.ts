...
@Component({
    ...
    standalone: true,
    template: `<ch-emoji-tabs [emojiCategories]="emojiCategories" [(selectedTab)]="selectedTab" (onTabClicked)="handleTabClicked($event)"></ch-emoji-tabs>`
})
...
describe('EmojiTabsComponent', () => {
    ...
    it('should render the correct number of category tabs', () => {
        const categoryButtons = fixture.debugElement.queryAll(By.css('ch-button'));
    ...
    it('should update selected category on click and emit event', () => {
        ...
        const categoryButton = fixture.debugElement.queryAll(By.css('.ch-button'))[2];

        categoryButton.triggerEventHandler('click', new MouseEvent('click'));
        fixture.detectChanges();

        expect(component.handleTabClicked).toHaveBeenCalledWith(category);

        // Ensure the selected category button has the active class
        expect(categoryButton.nativeElement.classList).toContain('ch-button-active');
    });
});
