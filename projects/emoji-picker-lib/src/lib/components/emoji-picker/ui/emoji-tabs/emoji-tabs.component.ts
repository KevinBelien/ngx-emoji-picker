...
import { HoverDirective, HoverEvent, ScreenService } from '@chit-chat/ngx-emoji-picker/src/lib/utils';
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, HoverDirective, IconComponent, ButtonComponent],
...
export class EmojiTabsComponent {
    ...
    protected handleHoverChange = (evt: HoverEvent, category: EmojiCategory) => {
        if (evt.isHovered) this.categoryHovered.set(category);
        else if (this.categoryHovered() === category) this.categoryHovered.set(null);
    };

    protected handleCategoryButtonClick = (evt: MouseEvent, category: EmojiCategory) => {
