...
describe('EmojiViewportComponent', () => {
	...
	beforeEach(() => {
		...
		component = fixture.debugElement.query(
			By.directive(EmojiViewportComponent)
		)?.componentInstance;
		viewportDebugElement = fixture.debugElement.query(
	...
	it('should scroll to the specified emoji category', () => {
		const calculateIndexOfCategorySpy = jest.spyOn(
			component as any,
			'calculateIndexOfCategory'
		);
		...
		expect(calculateIndexOfCategorySpy).toHaveBeenCalledWith(
			testCategory
		);
	...
	it('should emit onClick event when an emoji button is clicked', () => {
		const viewportDebugElement = fixture.debugElement.query(
			By.directive(CdkVirtualScrollViewport)
		);
		...
		viewportDebugElement.nativeElement.dispatchEvent(
			pointerDownEvent
		);
		fixture.detectChanges();
