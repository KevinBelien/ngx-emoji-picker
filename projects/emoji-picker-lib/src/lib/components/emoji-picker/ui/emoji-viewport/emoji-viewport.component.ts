...
import { AfterViewInit, ChangeDetectionStrategy, Component, computed, DestroyRef, HostBinding, inject, input, model, output, signal, viewChild } from '@angular/core';
import { emojis } from '../../data';
import { Emoji, emojiCategories, EmojiCategory, EmojiPickerRow, EmojiSize, EmojiSizeOption, FilteredEmojis, SuggestionEmojis } from '../../models';
...
import { ClickEvent, ClickTouchHoldDirective, NumberHelper, PreventContextMenuDirective, RippleDirective, TouchHoldEvent } from '@chit-chat/ngx-emoji-picker/src/lib/utils';
...
export class EmojiViewportComponent implements AfterViewInit {
    ...
    stickyHeaderCategory = signal<EmojiCategory>(this.emojiCategories()[0]);
    ...
    viewportWidth = computed(() => this.getViewportWidth(this.width(), this.emojiPickerService.padding(), this.scrollBarWidth()));

    emojiSizeInPx = computed(() => this.calculateEmojiSize(this.viewportWidth(), EmojiSize[this.emojiSize()], this.emojiPickerService.emojiItemSizeMultiplier()));

    itemSize = computed(() => NumberHelper.toFixedAndFloor(this.emojiSizeInPx() * this.emojiPickerService.emojiItemSizeMultiplier(), 2));

    suggestionEmojiRows = computed(() => {
        const suggestionEmojis = this.suggestionEmojis();
        if (!suggestionEmojis || !this.emojiCategories().includes('suggestions')) return [];

        return this.emojiPickerService.generateEmojiRows({
            emojiSize: this.emojiSizeInPx(),
            viewportWidth: this.viewportWidth(),
            itemSizeMultiplier: this.emojiPickerService.emojiItemSizeMultiplier(),
    ...
    emojiSizeInPx$ = zip([toObservable(this.emojiSizeInPx), toObservable(this.itemSize)])
        .pipe(takeUntilDestroyed())
    ...
    private initializeScrollerObserver = () => {
        this.viewport()
            ...
            .subscribe(() => {
                ...
                const currentCategoryRow = this.navigatedManually() || !previousRow ? currentRow : previousRow;

                this.navigatedManually.set(false);

                const stickyHeaderCategory = this.determineCurrentCategory(currentCategoryRow);
    ...
    private calculateEmojiSize(viewportWidth: number, emojiSize: EmojiSize, itemSizeMultiplier: number): number {
        return this.emojiPickerService.calculateEmojiSize(viewportWidth, emojiSize, itemSizeMultiplier);
    }
    ...
    private getViewportWidth = (width: number, padding: number, scrollbarWidth: number): number => width - scrollbarWidth - padding * 2;

    private getGlobalScrollbarWidth = (): number => {
        const root = document.querySelector(':root') as HTMLElement;
        const scrollbarWidth = getComputedStyle(root).getPropertyValue('--ch-scrollbar-size');
        return parseFloat(scrollbarWidth.replace('px', '').trim());
    };

    private handleNavigation = (index: number): void => {
        const targetNode = document.querySelector('.ch-static-category.ch-sticky.ch-row') as HTMLElement;
    ...
    private observeStyleChanges = (targetNode: HTMLElement, shouldDisconnect: () => boolean): void => {
    ...
    private calculateIndexOfCategory = (category: EmojiCategory): number => {
        const rows = this.emojiRows();
        return rows.findIndex((row) => row.type === 'category' && row.value === category);
    };
    ...
    private determineCurrentCategory(currentRow: EmojiPickerRow): EmojiCategory {
        return currentRow.type === 'emoji' ? currentRow.value[0].category : (currentRow.value as EmojiCategory);
    }
    ...
    protected handleWheelScroll(event: WheelEvent): void {
        ...
        viewport?.scrollToOffset(viewport?.measureScrollOffset() + scrollAmount);
    }
